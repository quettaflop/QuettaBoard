...
| Match key (cell) | workload profile \| target load \| actual concurrency | same |
...
concurrency 1 / 40 / 160. A fallback cell matches only when both sides used the same
actual concurrency. Profiles are averaged inside each domain (so the two Chat
...
**Decision.** `market.ts` carries the dated OpenRouter endpoint list-price range for
Llama-3.1-8B ($0.04–$0.29/M output across five endpoints as of 2026-09-13), regenerated
by `npm run refresh:market` from the public endpoints API. The page shows
*self-hosted floor ÷ API median* with the range and median of
...
percentage, page errors, or horizontal overflow. This browser check is part of
`build:site`; `scripts/check-site-bundle.mjs` separately guarantees the site bundle
...
|---|---|---|---|---|
| A100 40GB | 1 | 1.54× | $0.90 vs $1.39 | 9 matched models, better on 8, 30 configs, 434 cells |
| RTX 3090 | 2 | 1.13× | $0.73 vs $0.83 | 5 matched models, better on 3, 20 configs, 281 cells |
...
| SGLang 0.5.9 | 2 | 0.52× | $1.43 vs $0.75 | 10 matched models, better on 0, 44 configs, 626 cells |
...
- **Price.** Break-even used A100 price is **$3,554** (assumed $1,900); the 3090 ties at
  **$1,590** (assumed $1,275); alternatively H100 would need to fall to **$11,163** used.
...
claim reads "1.54× at $1,900 a card; ties at about $3,554".
