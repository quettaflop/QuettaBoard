    ...
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
      {METRICS.map((metric, idx) => {
        ...
        return (
          <div
            key={metric.key}
            className={`glass rounded-[22px] p-6 ${isOrphan ? 'xl:col-span-2' : ''}`}
          >
            <div className="mb-4">
              <h3 className="text-[15px] font-semibold tracking-tight text-[#f3f4f6]">{metric.label}</h3>
              <span className="text-[11px] text-[#676c76]">{metric.unit}</span>
            </div>
            ...
            <ResponsiveContainer width="100%" height={isOrphan ? 280 : 260}>
              ...
              >
                ...
                <YAxis
                  ...
                  axisLine={{ stroke: '#ffffff1f' }}
                  tickLine={{ stroke: '#ffffff1f' }}
                  label={{ value: metric.unit, angle: -90, position: 'insideLeft', fill: '#a9afba', fontSize: 11 }}
                />
                <Tooltip
